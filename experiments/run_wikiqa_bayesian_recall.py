#!/usr/bin/env python3
"""
🐟 markrel WikiQA Bayesian Optimization for RECALL

Uses Optuna for Bayesian optimization to maximize RECALL
with BAAI/bge-m3 embeddings.

Usage:
    python experiments/run_wikiqa_bayesian_recall.py
"""
...
def objective(trial, train_q_emb, train_d_emb, train_l, test_q_emb, test_d_emb, test_l):
    """Optuna objective function to MAXIMIZE RECALL."""
    ...
    try:
        ...
        # Find threshold that maximizes RECALL (use very low threshold to catch everything)
        ...
        # Objective: maximize recall with slight penalty for too many metrics
        num_metrics = len(metrics)
        speed_penalty = 0.005 * (num_metrics - 1)
        
        # Combined score (recall is primary)
        combined_score = best_recall - speed_penalty
...
def run_optimization(train, test, n_trials=100):
    """Run Bayesian optimization for RECALL."""
    
    print("=" * 70)
    print("🐟 markrel WikiQA Bayesian Optimization - MAXIMIZE RECALL")
    print("=" * 70)
    print(f"Embedding Model: BAAI/bge-m3 (1024-dim)")
    print(f"Optimization Trials: {n_trials}")
    print(f"Target: MAXIMIZE RECALL")
    print("=" * 70)
    ...
    # Run optimization
    print("\n🔬 Starting Bayesian Optimization (Recall)...")
    print("-" * 70)
    ...
    print("\n" + "=" * 70)
    print("📊 BEST HYPERPARAMETERS (Recall-Optimized)")
    print("=" * 70)
    ...
    print(f"\n📈 Final Results (Recall-Optimized):")
    ...
    study_path = os.path.join(
        os.path.dirname(__file__), "bayesian_optimization_recall_study.pkl"
    )
    ...
    json_path = os.path.join(
        os.path.dirname(__file__), "bayesian_optimization_recall_results.json"
    )
...
def main():
    ...
    print("\n" + "=" * 70)
    print("✅ Bayesian Optimization for RECALL Complete!")
    print("=" * 70)
